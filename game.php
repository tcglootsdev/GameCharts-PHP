...
<head>
	...
	<meta content="Game Detail Page. The name of this game is <?php echo($gameinfo_aux[0]->Name);?>  You will see more detailed info about this game in this page.<?php echo $dataURL; ?>" name="description" />
		...
		<span class="game-platforms-menu" style="margin-left: 0">
			...
			foreach($stores as $store) {
				if ($store->Store == $source){
					echo('<li><a href="http://gamecharts.local/'.$store->Store.'"><img alt="'.$store->Store.'" src="'.$store->Splash.'"/></a> </li>');
				}
			}
			?>
		</span>
