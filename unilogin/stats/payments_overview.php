...
$currency = empty($_GET['currency']) ? 'USD' : $_GET['currency'];
#########################################################################


#########################################################################
# GET RESUME GAME DATA
    $resumeGameData = statsPagosLoginFechaAmount($appid, $fechaOrigen, $fechaFinal, 1, $currency);
    $fieldName = 'tPagosLoginFechaRow';

# SUM TOTALS
// >> None Offer ####################################################
$totales = array();
$totales['PAGOS_DIAS'] = array();
$totales['PAGOS_SEMANAS'] = array();
$totales['PAGOS_MESES'] = array();
$totalPAGOS = 0;
foreach ($resumeGameData[$fieldName] as $key => $value) {

    foreach ($value->PAGOS_DIAS as $key2 => $value2) {
        
        $totales['PAGOS_DIAS'][$key2] = $totales['PAGOS_DIAS'][$key2]*1 + $value2*1;
    }
    
    foreach ($value->PAGOS_SEMANAS as $key2 => $value2) {
        
        $totales['PAGOS_SEMANAS'][$key2] = $totales['PAGOS_SEMANAS'][$key2]*1 + $value2*1;
    }
    
    foreach ($value->PAGOS_MESES as $key2 => $value2) {
        
        $totales['PAGOS_MESES'][$key2] = $totales['PAGOS_MESES'][$key2]*1 + $value2*1;
    }
    // $totalPAGOS = $totalPAGOS*1 + $value->NUSERS;
    $totalPAGOS = $totalPAGOS*1 + $value->PAGOS_PRIMERASEMANA*1
    			+ $value->PAGOS_PRIMERMES*1 + $value->PAGOS_MASMES*1;
}
/* <<== End of None Offer*/

/* ==>> Start of Offer Section */
$totales_offer = array();
$totales_offer['PAGOS_OFFER_DIAS'] = array();
$totales_offer['PAGOS_OFFER_SEMANAS'] = array();
$totales_offer['PAGOS_OFFER_MESES'] = array();
$totalPAGOS_offer = 0;
foreach ($resumeGameData[$fieldName] as $key => $value) {

    foreach ($value->PAGOS_OFFER_DIAS as $key2 => $value2) {
        
        $totales_offer['PAGOS_OFFER_DIAS'][$key2] = $totales_offer['PAGOS_OFFER_DIAS'][$key2]*1 + $value2*1;
    }
    
    foreach ($value->PAGOS_OFFER_SEMANAS as $key2 => $value2) {
        
        $totales_offer['PAGOS_OFFER_SEMANAS'][$key2] = $totales_offer['PAGOS_OFFER_SEMANAS'][$key2]*1 + $value2*1;
    }
    
    foreach ($value->PAGOS_OFFER_MESES as $key2 => $value2) {
        
        $totales_offer['PAGOS_OFFER_MESES'][$key2] = $totales_offer['PAGOS_OFFER_MESES'][$key2]*1 + $value2*1;
    }
    $totalPAGOS_offer = $totalPAGOS_offer*1 + $value->PAGOS_OFFER_PRIMERASEMANA*1
    			+ $value->PAGOS_OFFER_PRIMERMES*1 + $value->PAGOS_OFFER_MASMES*1;
}
...
$fullFW_offer_aux = array();
$fullFM_offer_aux = array();
$fullNM_offer_aux = array();

// FULLDATA
if (isset($resumeGameData['tPagosLoginFechaRow'])) {
    
    $temporal = (array)$resumeGameData['tPagosLoginFechaRow'];
    $fileResult = array();
    foreach ($temporal as $data)
    {
        $fullfecha_aux[] = $data->FECHA;
        $fullFW_aux[] = $data->PAGOS_PRIMERASEMANA*1;
        // $fullFW_aux[] = rand(100, 1000);
        $fullFM_aux[] = $data->PAGOS_PRIMERMES*1;
        $fullNM_aux[] = $data->PAGOS_MASMES*1;

        $fullFW_offer_aux[] = $data->PAGOS_OFFER_PRIMERASEMANA*1;
        $fullFM_offer_aux[] = $data->PAGOS_OFFER_PRIMERMES*1;
        $fullNM_offer_aux[] = $data->PAGOS_OFFER_MASMES*1;
    }
    ...
    $fullFW_Offer = json_encode($fullFW_offer_aux);
    $fullFM_Offer = json_encode($fullFM_offer_aux);
    $fullNM_Offer = json_encode($fullNM_offer_aux);
}
#########################################################################
...
#########################################################################
function statsPagosLoginFechaAmount($appid,$fechaOrigen,$fechaFinal,$groupby='1', $cMoneda='USD'){
...
<div class="row">
	...
	<div class="col-md-3">
		...
		<div class="form-group" id="currency_filter" <?php if (isset($_REQUEST['currency']) && !empty($_REQUEST['currency'])) { echo('style="display:block"'); }else{ echo('style="display:none"'); } ?>>
	        <div class="row">
	            <div class="col-md-12 alert alert-success">
	                
	                <strong><?php echo($currency); ?></strong>
	                
	                <button type="button" class="close" id="currency_close_filter" aria-label="Close">
					  <span aria-hidden="true">&times;</span>
					</button>
		...
		<div id="currency_select" <?php if (isset($_REQUEST['currency']) && !empty($_REQUEST['currency'])) { echo('style="display:none"'); }else{ echo('style="display:block"'); } ?>>
			<div class="form-group" style="width: 250px;">
	...
	<div class="col-sm-12">	
		...
		<table id="dtDropOff" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
		  <thead>
		  	...
		  	if (!empty($resumeGameData[$fieldName]) && count($resumeGameData[$fieldName])) {
		  	...
		  	<tr>
		  		...
		  		<th>
		  			<div>
		  			<?php echo(number_format(($totales['PAGOS_DIAS'][$i]/$totalPAGOS) * 100,2).'% <br/><span style="font-size:9px">('.$totales['PAGOS_DIAS'][$i].$currency.')</span>'); ?>
		  			</div>
		  			<div class="boder">
		  			<?php echo(number_format(($totales_offer['PAGOS_OFFER_DIAS'][$i]/$totalPAGOS_offer) * 100,2).'% <br/><span style="font-size:9px">('.$totales_offer['PAGOS_OFFER_DIAS'][$i].$currency.')</span>'); ?>
		  			</div>
		  		</th>		  			
		  		<?php }?>
		  		...
		  		<th>
		  			<div>
		  			<?php echo(number_format(($totales['PAGOS_SEMANAS'][$i]/$totalPAGOS) * 100,2).'% <br/><span style="font-size:9px">('.$totales['PAGOS_SEMANAS'][$i].$currency.')</span>'); ?>
		  			</div>
		  			<div class="boder">
		  			<?php echo(number_format(($totales_offer['PAGOS_OFFER_SEMANAS'][$i]/$totalPAGOS_offer) * 100,2).'% <br/><span style="font-size:9px">('.$totales_offer['PAGOS_OFFER_SEMANAS'][$i].$currency.')</span>'); ?>
		  			</div>
		  		</th>		  			
		  		<?php }?>
		  		...
		  		<th>
		  			<div>
		  			<?php echo(number_format(($totales['PAGOS_MESES'][$i]/$totalPAGOS) * 100,2).'% <br/><span style="font-size:9px">('.$totales['PAGOS_MESES'][$i].$currency.')</span>'); ?>	  				
		  			</div>
		  			<div class="boder">
		  			<?php echo(number_format(($totales_offer['PAGOS_OFFER_MESES'][$i]/$totalPAGOS_offer) * 100,2).'% <br/><span style="font-size:9px">('.$totales_offer['PAGOS_OFFER_MESES'][$i].$currency.')</span>'); ?>		  				
		  			</div>
		  		</th>		  			
		  		<?php }?>		  		
		  	</tr>
	  	...
	  	if (isset($resumeGameData[$fieldName]) && count($resumeGameData[$fieldName])) {
  	        ...
  	        echo('<tr>');
				...
				$day_total = $value->PAGOS_PRIMERASEMANA*1 + $value->PAGOS_PRIMERMES*1 + $value->PAGOS_MASMES*1;

				$day_total_offer = $value->PAGOS_OFFER_PRIMERASEMANA*1 + $value->PAGOS_OFFER_PRIMERMES*1 + $value->PAGOS_OFFER_MASMES*1;

  	            echo('<td>'.$value->FECHA.'<br/><div class="boder"><span style="font-size:9px">'.$day_total.$currency.'</div><div class="boder"><span style="font-size:9px">'.$day_total_offer.$currency.'</span></div></td>');
  	            ...
  	            foreach ($value->PAGOS_DIAS as $key2 => $value2) {
  	                ...
  	                $value2_offer = $value->PAGOS_OFFER_DIAS[$key2]*1;
  	                ...
  	                if (isset($value2)) {
  	                    ...
  	                    echo('<td>');
  	                    echo ('<div style="background-color: '.$background_color.';color: '.$color.'">');
  	                    echo($valor.' % <br/><span style="font-size:9px">('.$value2.$currency.')</span></div>');
  	                }
  	                ...
  	                if(isset($value2_offer)){
  	                    ...
  	                    echo ('<div class="boder" style="background-color: '.$background_color_offer.';color: '.$color_offer.'">');
  	                    echo($valor_offer.' % <br/><span style="font-size:9px">('.$value2_offer.$currency.')</span></div>');  	                    
  	                    echo('</td>');
  	                }
  	            ...
  	            foreach ($value->PAGOS_SEMANAS as $key2 => $value2) {
  	                ...
  	                $value2_offer = $value->PAGOS_OFFER_SEMANAS[$key2]*1;
  	            ...
  	            foreach ($value->PAGOS_MESES as $key2 => $value2) {
  	                ...
  	                $value2_offer = $value->PAGOS_OFFER_MESES[$key2]*1;
...
<!-- << Pay Amount Table -->
<?php if(!empty($resumeGameData['tExchangeRate']->tExchangeRateRow)):?>
<div class="row">
	<div class="col-md-1"></div>
	<div class="col-md-10">
		<h4 style="text-align: center;">Currency Exchange Rate View</h4>
		<table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
			...
			<tbody>
				<?php foreach($resumeGameData['tExchangeRate']->tExchangeRateRow as $row){?>
				<tr>
					<?php if($row->VALIDA == 0):?>
						<td style="background-color: #dbe8f5"><?= $currency ?></td>
						<td style="background-color: #dbe8f5">1</td>
						<td style="background-color: #dbe8f5"><?= date("Y-m-d H:i:s", strtotime($row->FECHAHORA)) ?></td>
					<?php else:?>
						<td><?= $row->MONEDA ?></td>
						<td><?= $row->CAMBIO ?></td>
						<td><?= date("Y-m-d H:i:s", strtotime($row->FECHAHORA))?></td>
					<?php endif;?>
				</tr>
				<?php }?>
			</tbody>
...
$( document ).ready(function() {
	...
	$('#search').click(function() {
		...
		window.location.href = "./payments_overview.php?appid="+appid+"&from="+fechaDesde+"&to="+fechaHasta+"&currency="+currency;		
	});
	...
	$('#currency_close_filter').click(function(){
		$('#currency_filter').hide();
	...
	Highcharts.chart('global', {
        ...
        title: {
            text: 'Pay Amount Graph Based on users paid date'
        },
        xAxis: {
        ...
        series: [
            ...
            },
            {
                ...
                data: <?php echo($fullNM_Offer); ?>,
                stack: "offer"       
            }, {
                name: 'First Month (Offer)',
                data: <?php echo($fullFM_Offer); ?>,
                stack: "offer" 
            }, {
                name: 'First Week (Offer)',
                data: <?php echo($fullFW_Offer); ?>,
                stack: "offer" 
            }
        ]
