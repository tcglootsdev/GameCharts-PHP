...
#########################################################################
$gamedataWS = statsCCU_TYW($fechaOrigen,$fechaFinal,$appid);
...
# GET RESUME GAME DATA
if (isset($gamedataWS['tJuegoResumen'])) {
	$gamedata_aux = $gamedataWS['tJuegoResumen'];
}

// OBTENEMOS LOS DATOS DE FULLDATA, TODAY, YESTERDAY, WEEKAGO
if (isset($gamedataWS['tJuegoMaxCcuDetalle'])){
	$fulldata_aux = $gamedataWS['tJuegoMaxCcuDetalle'];
}
/*$fulldata = str_replace(',[0000,0]','',file_get_contents($dataURL."fulldata.json"));
$fulldata_aux = json_decode($fulldata,true);*/

if (isset($gamedataWS['tJuegoAverageCcu'])){
	$fullaverage_aux = $gamedataWS['tJuegoAverageCcu'];
}
...
foreach ($fulldata_aux as $data) {
    ...
    $fecha = $months[$date[1]].$date[0];
    
    if (!isset($table_aux[$fecha])) {
        $table_aux[$fecha]['peak'] = $data->MAXIMO_CCU;
    }
    else {
        if ($table_aux[$fecha]['peak'] < $data->MAXIMO_CCU) {
            $table_aux[$fecha]['peak'] = $data->MAXIMO_CCU;
        }
    }
}
...
if (isset($fulldata_aux) && !empty($fulldata_aux)) {
	...
	$auxDate = explode('-',$data->FECHA);
	$auxHour = array($data->HORA);
        
        $aux[] = (int)(mktime($auxHour[0],'0','0',$auxDate[1],$auxDate[2],$auxDate[0])."000");
        $aux[] = (int)$data->MAXIMO_CCU;
...
if (isset($fullaverage_aux) && !empty($fullaverage_aux)) {
    ...
    foreach ($fullaverage_aux as $data)
    {
        ...
        $aux[] = (int)(mktime('0','0','0',$auxDate[1],$auxDate[2],$auxDate[0])."000");
        $aux[] = (int)$data->AVERAGE_CCU;
...
#########################################################################
function statsCCU_TYW($cFechaIni,$cFechaFin,$iIDJuego){
...
function statsCCUGeneral($cFechaIni,$cFechaFin,$iIDJuego){
    ...
    $retorno['result'] = $res->result;
    $retorno['tJuegoMaxCcuDetalle'] = $res->tJuegoMaxCcuDetalle->tJuegoMaxCcuDetalleRow;
    $retorno['tJuegoResumen'] = $res->tJuegoResumen->tJuegoResumenRow;
    $retorno['tJuegoAverageCcu'] = $res->tJuegoAverageCcu->tJuegoAverageCcuRow;
    return $retorno;
}
#########################################################################
?>
...
<div id="menu" class="row">
	<div class="app-stat col-sm-3"> 
		<span class="num"><?php echo($gamedata_aux->LAST_CCU); ?> ccu</span></span>
		<br>CURRENT
	</div>
	<div class="app-stat col-sm-3"> 
		<span class="num"><?php echo($gamedata_aux->TOP_CCU_24H); ?> ccu</span> / <span class="num"><?php echo((int)$gamedata_aux->MAX_AVG_24H); ?> average</span>
		<br>LAST 24h
	</div>
	<div class="app-stat col-sm-3"> 
		<span class="num"><?php echo($gamedata_au->TOP_CCU_30D); ?> ccu</span> / <span class="num"><?php echo((int)$gamedata_aux->MAX_AVG_30D); ?> average</span>
		<br>LAST 30d
	</div>
	<div class="app-stat col-sm-3"> 
		<span class="num"><?php echo($gamedata_aux->TOP_CCU); ?> ccu</span> / <span class="num"><?php echo((int)$gamedata_aux->MAX_AVG); ?> average</span>
		<br>ALL-TIME PEAK
	</div>
</div>
